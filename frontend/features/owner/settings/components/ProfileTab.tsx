...
function AvatarUpload({
  ...
  return (
    <button
      ...
      className={`group relative shrink-0 overflow-hidden focus:outline-none focus-visible:ring-3 focus-visible:ring-[#2d4a35]/40 ${
    ...
    >
      ...
      {/* Hover overlay (desktop) + persistent overlay (touch) */}
...
export function ProfileTab() {
  ...
  const profileFields: FieldDef[] = [
    ...
    {label: 'Email', value: user.email ?? '', save: v => persist({email: v})},
    {
      label: 'Phone number',
  ...
  const businessFields: FieldDef[] = [
    {
      ...
      save: v => persist({businessName: v})
    },
    {
      label: 'Store address',
      value: user.storeAddress ?? '',
      save: v => persist({storeAddress: v})
    },
    {
      label: 'Business contact number',
      value: user.businessContactNumber ?? '',
      save: v => persist({businessContactNumber: v})
    },
    {
      label: 'Operating hours',
      value: user.operatingHours ?? '',
      save: v => persist({operatingHours: v})
    },
    {
      label: 'Business type',
      value: user.businessType ?? '',
      save: v => persist({businessType: v})
    }
  ];
      ...
      >
        <div className="mb-6">
          <AvatarUpload
            ...
            fallback={(user.businessName || 'D').charAt(0)}
            onSave={url => persist({businessLogo: url})}
          />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
