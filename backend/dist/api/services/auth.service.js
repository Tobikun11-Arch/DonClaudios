...
export const authService = {
    ...
    async resendVerification(email) {
        ...
        const verificationCode = generateVerificationCode();
        const verificationExpiry = getVerificationExpiry(10);
        await userRepository.setVerificationCode(email, verificationCode, verificationExpiry);
        const emailTpl = verificationCodeEmailTemplate({
            code: verificationCode,
            expiresMinutes: 10,
    ...
    async resetPassword(email, code, newPassword) {
        ...
        await userRepository.updateProfile(customer.id, { passwordHash });
    },
    async login(email, password) {
        ...
        const accessToken = jwt.sign({ userId: customer.id, type: customer.type }, // type replaces role
        env.JWT_SECRET, { expiresIn: '15m' });
        const refreshToken = jwt.sign({ userId: customer.id }, env.JWT_REFRESH_SECRET, { expiresIn: '7d' });
    ...
    async refreshAccessToken(refreshToken) {
        try {
            ...
            const accessToken = jwt.sign({ userId: customer.id, type: customer.type }, env.JWT_SECRET, { expiresIn: '15m' });
            return { accessToken };
        }
        catch {
            throw new ApiError(401, 'UNAUTHORIZED', 'Invalid token');
